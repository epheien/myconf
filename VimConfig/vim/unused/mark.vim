...
"MOD8
hi link MarkWord1 HiWord1
hi link MarkWord2 HiWord2
...
if !hasmapto('<Plug>MarkRegex', 'v')
	xmap <unique> <silent> \r <Plug>MarkRegex
endif
...
function! s:MouseMark()
	...
	call <SID>MarkCurrentWord()
endf
nnoremap <silent> <2-LeftMouse> :call <SID>MouseMark()<CR><2-LeftMouse>

nnoremap <silent> <Plug>MarkSet   :call
	\ <sid>MarkCurrentWord()<cr>
xnoremap <silent> <Plug>MarkSet   <c-\><c-n>:call
	\ <sid>DoMark(<sid>GetVisualSelectionEscaped("enV"))<cr>
nnoremap <silent> <Plug>MarkRegex :call
	\ <sid>MarkRegex()<cr>
xnoremap <silent> <Plug>MarkRegex <c-\><c-n>:call
...
"			exe "syntax match MarkWord" . i . " " . quoted_regexp . " containedin=ALL"
			exe i . "match none"
			exe i . "match MarkWord" . i . " " . quoted_regexp
			return i
		endif
		let i = i + 1
	endwhile
