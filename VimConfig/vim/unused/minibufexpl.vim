...
" 
noremap <unique> <script> <Plug>MiniBufExplorer  
            \:call <SID>StartExplorer(1, -1, bufnr("%"))<CR>:<BS>
noremap <unique> <script> <Plug>CMiniBufExplorer 
            \:call <SID>StopExplorer(1)<CR>:<BS>
noremap <unique> <script> <Plug>UMiniBufExplorer 
            \:call <SID>AutoUpdate(-1,bufnr("%"))<CR>:<BS>
noremap <unique> <script> <Plug>TMiniBufExplorer 
            \:call <SID>ToggleExplorer()<CR>:<BS>
noremap <unique> <script> <Plug>MBEMarkCurrent 
            \:call <SID>MarkCurrentBuffer(bufname("%"),1)<CR>:<BS>
...
augroup MiniBufExplorer
    autocmd!
    autocmd BufDelete      * call <SID>DEBUG('-=> BufDelete AutoCmd', 10)
                \| call <SID>AutoUpdate(expand('<abuf>'),bufnr("%"))

    autocmd BufDelete      * call <SID>DEBUG(
                \   '-=> BufDelete ModTrackingListClean AutoCmd for buffer ' .
                \       bufnr("%"), 10)
                \| call <SID>CleanModTrackingList(bufnr("%"))

    autocmd BufEnter       * call <SID>DEBUG('-=> BufEnter AutoCmd', 10)
                \| call <SID>AutoUpdate(-1,bufnr("%"))

    autocmd BufEnter       * call <SID>DEBUG(
                \   '-=> BufEnter Checking for Last window', 10)
                \| call <SID>CheckForLastWindow()

    autocmd BufWritePost   * call <SID>DEBUG('-=> BufWritePost AutoCmd', 10)
                \| call <SID>AutoUpdate(-1,bufnr("%"))

    "autocmd CursorHold     * call <SID>DEBUG('-=> CursroHold AutoCmd', 10)
                "\| call <SID>AutoUpdateCheck(bufnr("%"))

    "autocmd CursorHoldI    * call <SID>DEBUG('-=> CursorHoldI AutoCmd', 10)
                "\| call <SID>AutoUpdateCheck(bufnr("%"))

    autocmd VimEnter       * call <SID>DEBUG('-=> VimEnter AutoCmd', 10)
      ...
      if g:miniBufExplSplitToEdge == 1 || a:forceEdge >= 0
          ...
          if l:edge
              if g:miniBufExplVSplit == 0
                  exec 'bo 1sp '.a:bufName
              else
                  exec 'bo vsp '.a:bufName
              endif
          else
              if g:miniBufExplVSplit == 0
                  exec 'to 1sp '.a:bufName
              else
...
function! <SID>MBESelectBuffer(split)
  ...
  if(l:bufnr != -1)             " If the buffer exists.
    ...
    if a:split == 0
        "exec('b! '.l:bufnr)
        call vlutils#OpenFile(bufname(l:bufnr))
    elseif a:split == 1
        exec('sb! '.l:bufnr)
    elseif a:split == 2
...
"      		          o You can open the selected buffer in a new vertically
...
"=============================================================================
" }}}
" vim:fdm=marker:ff=unix:tabstop=4:shiftwidth=4:softtabstop=4:expandtab
