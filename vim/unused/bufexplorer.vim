...
"    autocmd BufEnter * call s:BEActivateBuffer()
    ...
    autocmd BufWinEnter __BufExplorer__ call s:BEInitialize()
"    autocmd BufWinLeave __BufExplorer__ call s:BECleanup()
...
"    endif
    ...
    if s:running == 1
	  "MODs
	  if bufwinnr(bufnr("__BufExplorer__")) != -1
		let s:splitMode = ""
		exec bufwinnr(bufnr("__BufExplorer__")) . "wincmd w"
	  endif
...
"        return
  ...
  if (s:splitMode != "")
	...
	"MODs
	if s:splitMode == "sp" && exists("g:bufexplorerHorizontalInitHeight")
	  exec g:bufexplorerHorizontalInitHeight . "wincmd _"
	endif
...
function! s:BEOpen()
  ...
  exec bufwinnr("__BufExplorer__") . "wincmd w"
  exec "b" . l:bufnr
endfunction
...
"  nnoremap <buffer> <silent> <cr>          :call <SID>BESelectBuffer()<cr>
  nnoremap <buffer> <silent> <2-leftmouse> :call <SID>BEOpen()<cr>
  nnoremap <buffer> <silent> <cr>          :call <SID>BEOpen()<cr>
...
function! s:BESelectBuffer(...)
  ...
  if bufexists(_bufNbr)
    ...
    if (a:0 == 1) && (a:1 == "tab")
      ...
      " Restore __BufExplorer__ buffer.
...
"call s:BESet("g:bufExplorerMaxHeight", 25) " Handles dynamic resizing of the window.
  call s:BESet("g:bufExplorerMaxHeight", 20) " Handles dynamic resizing of the window.
endif
...
let s:splitMode = ""
let s:name = '__BufExplorer__'
let s:refreshBufferList = 1
let s:MRU_Exclude_List = ["__BufExplorer__","__MRU_Files__"]
"1}}}


"MODs
"仅仅负责进入缓冲区窗口时刷新已存在的 BufExplorer 窗口
function! BufExplorer_Fix()
  let l:BEWinnr = bufwinnr(bufnr("__BufExplorer__"))
  ...
  let l:curBuf = expand("<abuf>")
"  echo l:curBuf
  if buflisted(l:curBuf)
	call search('^\s*' . l:curBuf, "c")
  else
	call search('^\s*\d\+.\{2}a.*', "c")
  endif
  normal! zz
  ...
  exec l:orgWin . "wincmd w"
endf
autocmd BufWinEnter * call BufExplorer_Fix()
