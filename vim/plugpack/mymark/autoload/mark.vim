...
function! s:MarkMatch( indices, expr )
	...
	if ! empty(a:expr)
		...
		" MOD: add local ignore case support by fanhe
		let l:expr = ((g:mwIgnoreCase && a:expr !~# '\\\@<!\\C') ? '\c' . a:expr : '\C' . a:expr)
	...
	while i >= 0
		if ! empty(s:pattern[i])
			...
			while start >= 0 && start < strlen(line) && start < col('.')
				...
				if b < col('.') && col('.') <= e
					let &ignorecase = l:save_ignorecase " restore 'ignorecase'
...
function! s:Search( pattern, isBackward, currentMarkPosition, searchType )
	...
	set nosmartcase
	let l:pattern = ((g:mwIgnoreCase && a:pattern !~# '\\\@<!\\C') ? '\c' . a:pattern : '\C' . a:pattern)
	...
	while l:count > 0
		...
		let [l:line, l:col] = searchpos( l:pattern, (a:isBackward ? 'b' : '') )
...
"****D echomsg '****' l:line l:isStuckAtCurrentMark l:isWrapped l:isMatch string([l:line, l:col]) string(a:currentMarkPosition)
	if l:line > 0 && ! l:isStuckAtCurrentMark
		...
		if l:isWrapped
			call s:WrapMessage(a:searchType, l:pattern, a:isBackward)
		else
			call s:EchoSearchPattern(a:searchType, l:pattern, a:isBackward)
		endif
		return 1
	else
		if l:isMatch
		...
		if l:line > 0 && l:isStuckAtCurrentMark && l:isWrapped
			...
			call s:ErrorMessage(a:searchType, l:pattern, a:isBackward)
			return 0
		endif
	endif
endfunction
